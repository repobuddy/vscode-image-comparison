...
export class ImageComparePanel {
	...
	private constructor(panel: vscode.WebviewPanel, extensionUri: vscode.Uri, images: ImagePair) {
		this.panel = panel
	...
	static async createOrShow(extensionUri: vscode.Uri, images: ImagePair): Promise<void> {
	...
	private async update(images: ImagePair): Promise<void> {
		...
		this.panel.title = `Image Comparison: ${path.basename(images[0].fsPath)} ↔ ${path.basename(images[1].fsPath)}`
